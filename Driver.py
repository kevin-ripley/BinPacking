...
"""
EVALUATE_SOLUTION:
    Find the area of the rectangle that contains the packing.
--------------------------------------------------
...
--------------------------------------------------
RETURNS: the area of the bounding rectangle, an integer
"""
...
def evaluate_solution (coordinates):
    ...
    return bounding_rectangle_area
...
def solve_problem(file_name):
    ...
    start = time.time()
    upper_left_coordinates = BinPacking.find_solution(clone)
    ...
    naive_area = evaluate_solution(naive_rectangle_coordinates)
    print("Area of Naive Solution =", naive_area)


    if is_solution_valid (rectangle_coordinates):   # is student solution valid?
        area = evaluate_solution(rectangle_coordinates)
        print("Area of Your Solution =", area)
        if time_elapsed > 5.0:                      # is student solution fast enough?
            print("Error.  Time Limit Exceeded.")
            area = 2 * naive_area                   # answer is penalized
        
    else:
        print("Error.  Overlapping Rectangles in Solution.")
        area = 2 * naive_area                       # answer is penalized

    print("Percentage Improvement Over Naive Solution =", 100 - (area / naive_area) * 100)
